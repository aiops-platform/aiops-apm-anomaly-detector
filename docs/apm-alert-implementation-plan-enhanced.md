...
class AppException(Exception):
    def __init__(self, code: ErrorCode, reason: str, trace_id: str | None = None):
        self.code = code
        self.reason = reason
...
async def lifespan(app: FastAPI):
    # startup: 初始化连接池、加载插件、启动 scheduler（后续阶段填充）
    yield
    # shutdown: 关闭连接池、停止 scheduler

def create_app(settings: Settings) -> FastAPI:
    app = FastAPI(title="APM Alert Module", lifespan=lifespan)
    app.state.settings = settings
    # 后续阶段填充: registry, storage, scheduler
...
async def ready(request: Request):
    ...
    if not ok:
        return JSONResponse(
            status_code=503,
            content={"code": "NOT_READY", "reason": str(checks)}
        )
...
流程:
  1. 运维执行 uvicorn 启动命令
  2. FastAPI 调用 create_app(settings) 构建应用实例
  3. lifespan startup 钩子执行（M0 阶段为空壳）
  4. 服务监听 0.0.0.0:8000
  5. GET /health → 200 {"status": "ok"}
  6. GET /ready → 503（无 DB 连接）或 200（有 DB）
断言: /health 恒 200；/ready 在 DB 未连接时返回 503 + {code, reason}
```
...
流程:
  1. 设置 APM_PORT=9090
  2. 启动服务
  3. Settings 自动读取 APM_PORT
  4. 服务监听 9090
断言: settings.port == 9090
```
...
流程:
  1. 请求一个不存在的资源
  2. 异常处理中间件捕获 → AppException(code=NOT_FOUND, reason="...")
  3. 返回 {"code": "NOT_FOUND", "reason": "...", "trace_id": "..."}
断言: 所有异常返回统一 JSON 结构，包含 code/reason/trace_id
```

---

## M1 — 契约层（Pydantic 模型 + fingerprint 真源）
...
- **目标**：单 schema 的表全部可读写，v1 表集一次建齐（含 P0 改动后的列）
- **依赖**：M1
- **产出**：`make migrate` 建齐单 schema 所有表；所有 store 可读写
- **完成标准**：迁移幂等可重入；并发 `write_or_append` 同 `group_key` 只产生一条记录；所有 store 无 `tenant_id` 入参则抛 `ValueError`
- **注意**：M2 与 M3/M4 可并行启动设计，但实现合并需 M1 先冻结

### 前端菜单与页面

| 菜单路径 | 页面 | 组件 | 说明 |
|---------|------|------|------|
| 系统 > 数据库迁移 | MigrationPage | `MigrationStatus` 表格 + `MigrateButton` | 展示当前 schema 版本号、各表建表状态；一键执行 `make migrate` 等价操作（通过 API 触发） |
| 系统 > 数据库状态 | DBStatusPage | `DBConnection` 状态卡片 | 展示连接池状态、各表行数、慢查询计数 |

> M2 前端为运维管理类页面，不对外暴露给普通用户。
...
class MigrationRunner:
    ...
    async def ensure_schema_versions(self):
    ...
    async def migrate(self):
        ...
        for script in sorted(self._load_scripts(), key=lambda s: s.version):
            if script.version > current:
                await self._execute(script)
                await self._record_version(script.version)
...
class RecordStore(ABC):
    ...
    async def find_open(self, tenant_id: str, group_key: str) -> dict | None:
        """查找同 group_key 且 state 为 open 的记录。"""
        ...

    @abstractmethod
    async def write_or_append(
        self, tenant_id: str, record: ProblemRecord
    ) -> str:
        """原子开单或追加 evidence（INSERT ... ON DUPLICATE KEY UPDATE）。"""
        ...
    ...
    async def list(
        ...
        """查询 problem_record，强制 tenant_id。"""
        ...

    @abstractmethod
    async def resolve(
        self, tenant_id: str, record_id: str, reason: str = "auto"
    ) -> bool:
        """标记为已解决。"""
        ...
...
class InMemoryRecordStore(RecordStore):
    ...
    async def write_or_append(self, tenant_id: str, record: ProblemRecord) -> str:
        existing = await self.find_open(tenant_id, record.group_key)
        if existing:
            existing["evidence"].extend(record.evidence)
            existing["occurrence_count"] += 1
            existing["last_seen_at"] = record.detected_at
            if record.severity > existing.get("severity", "warning"):
                existing["severity"] = record.severity
            return existing["record_id"]
        else:
            self._records[record.record_id] = record.model_dump()
            return record.record_id

class MySQLRecordStore(RecordStore):
    """依赖 open_group_key 生成列 + UNIQUE 实现原子去重。"""
    # INSERT INTO problem_record (...) VALUES (...)
    # ON DUPLICATE KEY UPDATE
    #   evidence = JSON_ARRAY_APPEND(evidence, '$', VALUES(evidence)),
    #   occurrence_count = occurrence_count + 1,
    #   last_seen_at = VALUES(detected_at),
...
class MonitorTargetStore(ABC):
    @abstractmethod
    async def list(self, tenant_id: str, enabled_only: bool = True) -> list[dict]: ...
    @abstractmethod
    async def get(self, tenant_id: str, target_id: str) -> dict | None: ...
    @abstractmethod
    async def create(self, tenant_id: str, target: dict) -> str: ...
    @abstractmethod
    async def update(self, tenant_id: str, target_id: str, patch: dict) -> bool: ...
    @abstractmethod
    async def delete(self, tenant_id: str, target_id: str) -> bool: ...
...
# storage/state_store.py —— detection_state（v2→v1）
class DetectionStateStore(ABC):
    @abstractmethod
    async def get(self, tenant_id: str, domain: str, anomaly_key: str) -> dict | None: ...
    @abstractmethod
    async def upsert(self, tenant_id: str, domain: str, anomaly_key: str,
                     first_seen: datetime, last_seen: datetime,
                     consecutive_rounds: int, miss_rounds: int) -> None: ...
    @abstractmethod
    async def list_by_domain(self, tenant_id: str, domain: str) -> list[dict]: ...

# storage/snapshot_store.py
class SnapshotStore(ABC):
    @abstractmethod
    async def write(self, tenant_id: str, target_id: str,
                    signals: list) -> None: ...
    @abstractmethod
    async def read_window(self, tenant_id: str, target_id: str,
                          metric: str, since: datetime, limit: int = 100) -> list: ...

# storage/watermark_store.py
class WatermarkStore(ABC):
    @abstractmethod
    async def get(self, tenant_id: str, target_id: str) -> dict | None: ...
    @abstractmethod
    async def update(self, tenant_id: str, target_id: str,
                     last_ts: datetime, last_cursor: str | None = None) -> None: ...

# storage/sequence_store.py
class SequenceStore(ABC):
    @abstractmethod
    async def next_id(self, domain: str) -> str:
        """事务内原子取号: PR-YYYYMMDD-NNNN"""
        ...

# config/loader.py
class DomainConfigLoader:
    def __init__(self, store: DomainConfigStore, yaml_seed_path: str | None = None):
        self.store = store
        self.yaml_seed_path = yaml_seed_path
        self._cache: list[dict] | None = None  # last-known-good
    ...
    async def load(self, tenant_id: str) -> list[dict]:
        try:
            ...
            if self._cache:
                return self._cache  # 回退 last-known-good
            raise
...
```
前置: MySQL 已启动，M1 契约已冻结
流程:
  1. 执行 make migrate（或调用 MigrationRunner.migrate()）
  2. MigrationRunner 检查 schema_versions 表是否存在
  3. 读取当前版本号 current_version
  4. 加载 migrations/ 目录下所有 SQL 脚本
  5. 按 version 排序，执行 > current_version 的脚本
  6. 每个脚本执行成功后记录版本号
  7. aiops_apm_runtime schema（单一库）: problem_record（含 severity/open_group_key 等）、
     change_record、domain_config、monitor_target、maintenance_window、
     suppress_blacklist、fpr_table、record_seq、scheduler_lease、
     detection_state、signal_snapshot、detection_round
断言: 迁移幂等可重入（再次执行不报错）；所有表存在且列正确
```

#### UC-2.2 新开 problem_record

```
前置: M2 已完成迁移，RecordStore 可用
流程:
  1. 构造 ProblemRecord（record_id, group_key, state=pending, ...）
  2. 调用 store.write_or_append(tenant_id, record)
  3. store.find_open(tenant_id, group_key) → 未命中
  4. 执行 INSERT，生成列 open_group_key = group_key
  5. 返回 record_id
断言: problem_record 表新增一行；open_group_key = group_key；state = pending
```

#### UC-2.3 追加 evidence（去重命中）

```
前置: 存在同 group_key 的 open record
流程:
  1. 构造新的 ProblemRecord（同 group_key，不同 evidence）
  2. 调用 store.write_or_append(tenant_id, record)
  3. find_open 命中已有 record
  4. 执行 INSERT ... ON DUPLICATE KEY UPDATE
  5. evidence 追加新证据；occurrence_count++；last_seen_at 更新
  6. 如新 severity 更高则升级
断言: 记录数不变；occurrence_count 递增；evidence 包含新证据；last_seen_at 已更新
```

#### UC-2.4 已关闭记录复发开单

```
前置: 存在同 group_key 但 state=resolved 的记录
流程:
  1. 构造 ProblemRecord（同 group_key）
  2. 调用 write_or_append
  3. find_open → 未命中（因为 open_group_key 在 state=resolved 时为 NULL）
  4. INSERT 成功，新开一条 record
断言: 表中新增一行；新记录 record_id 与旧记录不同；state=pending
```

#### UC-2.5 配置加载与 Seed

```
前置: domain_config 表为空，domains.yaml 存在
流程:
  1. DomainConfigLoader.load(tenant_id="default")
  2. store.load("default") → 空列表
  3. 读取 config/domains.yaml seed
  4. store.seed("default", seed) → INSERT ... ON DUPLICATE KEY UPDATE
  5. 再次 load → 返回 seed 数据
  6. 保存到 _cache（last-known-good）
断言: 首次加载 seed 幂等写入；再次加载从 DB 读取；seed 结果与 YAML 一致
```

#### UC-2.6 配置加载失败回退

```
前置: DB 连接断开，_cache 有 last-known-good
流程:
  1. DomainConfigLoader.load("default")
  2. store.load 抛 ConnectionError
  3. 捕获异常，返回 _cache（上一版配置）
断言: 服务不崩溃；返回上一版配置；日志记录降级事件
```
