...
**项目全貌一页卡：** 新会话 / 问答 / enhance 先读 [`docs/project-overview.md`](docs/project-overview.md)——5 分钟了解全貌、当前状态、续接点与已知边界；端到端手动跑通手册见 [`docs/operational-guide.md`](docs/operational-guide.md)。
...
2. **可插拔规则** — 采集器、检测器、抑制器通过 Python `entry_points` 动态加载；配置在 PostgreSQL 中以「插件名 + 参数」引用。改配置即热插拔，无需重启。
...
- **技术栈**：Python 3.10+、FastAPI（uvicorn 启动，监听 `:8000`）、PostgreSQL（`psycopg3`，M8 起取代 MySQL/`aiomysql`）、`pydantic`/`pydantic-settings`、`prometheus_client`。
...
- **存储** — PostgreSQL，全部表建在**独立 schema** `aiops_apm_runtime` 下（库由 `db_name` 指定，默认复用 multi-agent-workflow 的 `agentflow` 库；schema 隔离使其与同库其它服务的表零冲突）：
...
- `make migrate` — 建齐 PG schema（`aiops_apm_runtime`）所有表
- `make test-pg APM_TEST_PG_DSN=...` — 真库集成道（不设 DSN 时 `make test` 自动 skip 该文件）
